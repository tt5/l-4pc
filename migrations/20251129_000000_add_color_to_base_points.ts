...
export const name = '20251129_000000_add_color_to_base_points';

export async function up(db: Database): Promise<void> {
  console.log('Adding color column to base_points table...');
  
  // Add color column with a default color
  await db.exec(`
    ALTER TABLE base_points
    ADD COLUMN color TEXT NOT NULL DEFAULT '#4CAF50';
  `);
...
export async function down(db: Database): Promise<void> {
  console.log('Reverting color column addition...');
  ...
  await db.exec(`
    ...
    INSERT INTO base_points_backup (id, user_id, x, y, created_at_ms, updated_at_ms)
    SELECT id, user_id, x, y, created_at_ms, updated_at_ms
    FROM base_points;
